...
const CONFIG = {
  delayMs: 500,            // Delay before the cue starts
  durationMs: 3500,        // Total duration for the entire sequence
  image: {
  ...
  return (
    <AnimatePresence>
      <motion.div className="fixed inset-0 pointer-events-none z-40" initial={{ opacity: 1 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
        ...
        */}
        <motion.div
          style={{ position: 'fixed', left: blockLeft, top: blockTop, width: imageSize, pointerEvents: 'auto', cursor: 'pointer' }}
        ...
        <motion.div
          ...
          style={{ position: 'fixed', left: blockLeft - 20, top: blockTop + imageSize - 60, width: 12, height: 12, pointerEvents: 'none' }}
        ...
        <motion.div
          className="bg-brand-secondary rounded-full"
          style={{ position: 'fixed', left: blockLeft - 50, top: blockTop + imageSize - 80, width: 18, height: 18, pointerEvents: 'none' }}
          initial={{ opacity: 0, scale: 0.6 }}
          animate={{ opacity: [0, 1, 1, 0], scale: [0.6, 1, 1, 0.6] }}
        ...
        <motion.div
          className="bg-brand-secondary rounded-full"
          style={{ position: 'fixed', left: blockLeft - 85, top: blockTop + imageSize - 115, width: 36, height: 36, pointerEvents: 'none' }}
          initial={{ opacity: 0, scale: 0.6 }}
        ...
        {/* Circle around icon */}
        <motion.div
          style={{ position: 'fixed', left: iconCenter.x - 20, top: iconCenter.y - 20, width: 40, height: 40, borderRadius: 9999, borderWidth: 3, borderColor: 'rgb(64,164,76)', pointerEvents: 'none' }}
